...
export const getEventProposals = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Erreur lors de la récupération des propositions d'événements",
      });
  }
};
...
export const getEventProposalById = async (req, res) => {
  try {
    ...
    if (!proposal) {
      return res
        .status(404)
        .json({ message: "Proposition d'événement non trouvée" });
    }
    ...
    res
      .status(500)
      .json({
        message: "Erreur lors de la récupération de la proposition d'événement",
      });
  }
};
...
export const createEventProposal = async (req, res) => {
  try {
    const {
      ...
      proposeur_nom,
      proposeur_telephone,
    } = req.body;    // Récupérer l'URL de l'affiche si un fichier a été uploadé
    ...
    res
      .status(500)
      .json({
        message: "Erreur lors de la création de la proposition d'événement",
      });
  }
};
...
export const processEventProposal = async (req, res) => {
  try {
    ...
    if (statut === "approuve") {
      ...
      let createur = await Utilisateur.findOne({
        where: { email: proposal.proposeur_email },
      });
      ...
      const TracageEvenement = (await import("../models/tracageEvenement.js"))
        .default;
      await TracageEvenement.create({
        evenement_id: newEvent.id,
        titre: proposal.titre, // Ajout du titre du proposal
        action: "acceptation",
        user_id: req.user.id,
        proposeur_nom: proposal.proposeur_nom,
        proposeur_email: proposal.proposeur_email,
        affiche_url: proposal.affiche_url,
      ...
      await sendEmail({
        ...
        },\n\nNous sommes heureux de vous informer que votre proposition d'événement "${
          proposal.titre
        }" a été approuvée.\n\n${
          commentaire_admin ? `Commentaire : ${commentaire_admin}\n\n` : ""
        }L'événement a été programmé du ${new Date(
          proposal.date_debut
      ...
      await sendEmail({
        ...
        text: `Bonjour ${
          proposal.proposeur_nom
        },\n\nNous vous remercions pour votre proposition d'événement "${
          proposal.titre
    ...
    res.json({
      success: true,
      message: `Proposition d'événement ${
